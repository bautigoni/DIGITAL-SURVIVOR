...
const VERIFICATION_TIPS: Partial<Record<ThreatCategory, string>> = {
...
const FRAMINGS: Partial<Record<ThreatCategory, string[]>> = {
  phishing: [
...
const NEGATIVE_DELTAS: Partial<Stats> = {
  security: -20,
  money: -20,
  reputation: -15,
  trust: -10,
...
const POSITIVE_DELTAS: Partial<Stats> = {
  security: 8,
  knowledge: 12,
  reputation: 5,
  stress: -5,
...
const EDUCATIONAL_DELTAS: Partial<Stats> = {
  security: 12,
  knowledge: 15,
  reputation: 8,
  stress: -5,
...
const generateSafe = (rng: Rng, cat: ThreatCategory): Choice => ({
...
const generateRisky = (rng: Rng, cat: ThreatCategory): Choice => ({
...
const generateDangerous = (rng: Rng, cat: ThreatCategory): Choice => ({
...
const generateEducational = (rng: Rng, cat: ThreatCategory): Choice => ({
...
export const generateProceduralEvent = (opts: ProceduralOptions): GameEvent => {
...
export interface PickOptions {
  ...
  playerLevel: number;
  recentIds?: readonly string[];
  /** Probabilidad de generar procedural vs. tomar del banco. 0..1 */
  proceduralChance?: number;
}

const CATEGORIES: readonly ThreatCategory[] = [
  'phishing',
  'fake_news',
  'malware',
  'disinformation',
 ...
 */
export const pickEvent = (opts: PickOptions): GameEvent => {
  const { rng, zone, playerLevel, recentIds = [], proceduralChance = 0.4 } = opts;
  const pool = SEED_EVENTS.filter(
    (e) => e.zoneId === zone && e.minLevel <= playerLevel && !recentIds.includes(e.id),
  );
